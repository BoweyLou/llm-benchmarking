...
class ReviewWorkbenchTests(unittest.TestCase):
    ...
    def test_review_app_and_catalog_are_readable(self) -> None:
        ...
        self.assertIn("policySuggestions", app_response.text)
        self.assertIn("TRUSTED_PROVIDER_POLICY", app_response.text)
        self.assertIn("trusted_provider_approval", app_response.text)
        self.assertIn("stale_trusted_versions", app_response.text)
        self.assertIn("recentVersionDepth: 3", app_response.text)
