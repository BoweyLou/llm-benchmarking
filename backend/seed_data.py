"""Seed data and idempotent bootstrap helpers."""

from __future__ import annotations

from collections.abc import Iterable, Mapping
import json
from pathlib import Path
import re
from typing import Any

from sqlalchemy import func, select
from sqlalchemy.dialects.sqlite import insert as sqlite_insert
from sqlalchemy.engine import Connection, Engine

from .database import benchmarks, models, providers, scores, utc_now_iso

PROVIDER_ORIGIN_VERIFIED_AT = "2026-04-02T00:00:00Z"
PROVIDER_ORIGIN_BASELINE_PATH = Path(__file__).with_name("provider_origin_baseline.json")
INTERNAL_VIEW_BENCHMARK_ID = "internal_view"
AA_MODELS_URL = "https://artificialanalysis.ai/leaderboards/models"

PROVIDER_CANONICAL_NAME_ALIASES = {
    "amazon": "Amazon",
    "amazonbedrock": "Amazon",
    "amazonnova": "Amazon",
    "amazonwebservices": "Amazon",
    "aws": "Amazon",
    "azure": "Microsoft",
    "azureaifoundry": "Microsoft",
    "azureopenai": "Microsoft",
    "alibabacloud": "Alibaba",
    "ibmgranite": "IBM",
    "microsoft": "Microsoft",
    "microsoftazure": "Microsoft",
    "mistral": "Mistral AI",
    "mistralai": "Mistral AI",
    "qwen": "Alibaba",
    "qwenteam": "Alibaba",
}


def _provider_alias_key(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", value.strip().lower())


def canonical_provider_name(name: str | None) -> str:
    cleaned = str(name or "").strip()
    if not cleaned:
        return cleaned
    return PROVIDER_CANONICAL_NAME_ALIASES.get(_provider_alias_key(cleaned), cleaned)


def _aa_models_benchmark(
    benchmark_id: str,
    name: str,
    short: str,
    category: str,
    metric: str,
    *,
    higher_is_better: int = 1,
    tier: int = 2,
    description: str,
) -> dict[str, Any]:
    return {
        "id": benchmark_id,
        "name": name,
        "short": short,
        "source": "Artificial Analysis",
        "url": AA_MODELS_URL,
        "category": category,
        "metric": metric,
        "higher_is_better": higher_is_better,
        "tier": tier,
        "scraper_id": "ArtificialAnalysisAdapter",
        "description": description,
        "active": 1,
    }


AA_MODELS_EXTRA_BENCHMARKS: list[dict[str, Any]] = [
    _aa_models_benchmark(
        "aa_coding_index",
        "AA Coding Index",
        "AA Coding",
        "Coding",
        "Index Score",
        tier=1,
        description="Artificial Analysis coding capability index from the public models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_agentic_index",
        "AA Agentic Index",
        "AA Agentic",
        "Agentic & Tool Use",
        "Index Score",
        tier=1,
        description="Artificial Analysis agentic capability index from the public models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_tau2_telecom",
        "AA tau2 Telecom",
        "AA tau2",
        "Agentic & Tool Use",
        "Success %",
        description="Artificial Analysis tau2 telecom task score.",
    ),
    _aa_models_benchmark(
        "aa_tau_banking",
        "AA Tau Banking",
        "AA Banking",
        "Agentic & Tool Use",
        "Success %",
        description="Artificial Analysis banking agent task score.",
    ),
    _aa_models_benchmark(
        "aa_terminalbench_hard",
        "AA Terminal-Bench Hard",
        "AA TBench H",
        "Agentic & Tool Use",
        "Accuracy %",
        description="Artificial Analysis Terminal-Bench Hard score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_terminalbench_v2_1",
        "AA Terminal-Bench v2.1",
        "AA TBench",
        "Agentic & Tool Use",
        "Accuracy %",
        description="Artificial Analysis Terminal-Bench v2.1 score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_scicode",
        "AA SciCode",
        "AA SciCode",
        "Coding",
        "Accuracy %",
        description="Artificial Analysis SciCode score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_lcr",
        "AA Long Context Reasoning",
        "AA LCR",
        "Long Context",
        "Accuracy %",
        description="Artificial Analysis long-context reasoning score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_omniscience_index",
        "AA Omniscience Index",
        "AA Omni",
        "Knowledge & Hallucination",
        "Index Score",
        description="Artificial Analysis Omniscience index from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_omniscience_accuracy",
        "AA Omniscience Accuracy",
        "AA Omni Acc",
        "Knowledge & Hallucination",
        "Accuracy %",
        description="Artificial Analysis Omniscience accuracy companion metric.",
    ),
    _aa_models_benchmark(
        "aa_omniscience_non_hallucination",
        "AA Omniscience Non-Hallucination",
        "AA NonHall",
        "Knowledge & Hallucination",
        "Non-hallucination %",
        description="Artificial Analysis Omniscience non-hallucination companion metric.",
    ),
    _aa_models_benchmark(
        "aa_hle",
        "AA Humanity's Last Exam",
        "AA HLE",
        "Reasoning & Knowledge",
        "Accuracy %",
        description="Artificial Analysis Humanity's Last Exam score.",
    ),
    _aa_models_benchmark(
        "aa_gpqa_diamond",
        "AA GPQA Diamond",
        "AA GPQA",
        "Reasoning & Math",
        "Accuracy %",
        description="Artificial Analysis GPQA Diamond score. Kept separate from the Epoch AI GPQA source.",
    ),
    _aa_models_benchmark(
        "aa_critpt",
        "AA CritPt",
        "AA CritPt",
        "Coding",
        "Accuracy %",
        description="Artificial Analysis CritPt score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_gdpval_normalized",
        "AA GDPval Normalized",
        "AA GDPval",
        "Business & Professional",
        "Normalized %",
        description="Artificial Analysis GDPval-AA normalized score.",
    ),
    _aa_models_benchmark(
        "aa_mmmu_pro",
        "AA MMMU-Pro",
        "AA MMMU-Pro",
        "Multimodal",
        "Accuracy %",
        description="Artificial Analysis MMMU-Pro score. Kept separate from the official MMMU source.",
    ),
    _aa_models_benchmark(
        "aa_apex_agents",
        "AA Apex Agents",
        "AA Apex",
        "Agentic & Tool Use",
        "Score %",
        description="Artificial Analysis Apex Agents score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_itbench_sre",
        "AA ITBench SRE",
        "AA ITBench",
        "Agentic & Tool Use",
        "Score %",
        description="Artificial Analysis ITBench SRE score from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_openness_index",
        "AA Openness Index",
        "AA Open",
        "Openness",
        "Index Score",
        tier=3,
        description="Artificial Analysis openness index from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_multilingual_index",
        "AA Multilingual Index",
        "AA Multi",
        "Multilingual",
        "Index %",
        tier=3,
        description="Artificial Analysis multilingual index from the models leaderboard payload.",
    ),
    _aa_models_benchmark(
        "aa_cost_blended_7_2_1",
        "AA Cost Blended 7:2:1",
        "Cost 7:2:1",
        "Latency & Cost",
        "$/1M tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis blended 7:2:1 token pricing companion metric.",
    ),
    _aa_models_benchmark(
        "aa_cost_blended_1_1",
        "AA Cost Blended 1:1",
        "Cost 1:1",
        "Latency & Cost",
        "$/1M tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis blended 1:1 token pricing companion metric.",
    ),
    _aa_models_benchmark(
        "aa_cost_blended_100_1",
        "AA Cost Blended 100:1",
        "Cost 100:1",
        "Latency & Cost",
        "$/1M tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis input-heavy blended pricing companion metric.",
    ),
    _aa_models_benchmark(
        "aa_cost_blended_output_heavy",
        "AA Cost Blended Output Heavy",
        "Cost Out",
        "Latency & Cost",
        "$/1M tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis output-heavy blended pricing companion metric.",
    ),
    _aa_models_benchmark(
        "aa_price_input",
        "AA Input Price",
        "AA Input $",
        "Latency & Cost",
        "$/1M input tokens",
        higher_is_better=0,
        tier=2,
        description="Artificial Analysis input-token price.",
    ),
    _aa_models_benchmark(
        "aa_price_output",
        "AA Output Price",
        "AA Output $",
        "Latency & Cost",
        "$/1M output tokens",
        higher_is_better=0,
        tier=2,
        description="Artificial Analysis output-token price.",
    ),
    _aa_models_benchmark(
        "aa_price_cache_hit",
        "AA Cache-Hit Price",
        "AA Cache Hit",
        "Latency & Cost",
        "$/1M cache-hit tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis cache-hit token price.",
    ),
    _aa_models_benchmark(
        "aa_price_cache_write",
        "AA Cache-Write Price",
        "AA Cache Write",
        "Latency & Cost",
        "$/1M cache-write tokens",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis cache-write token price.",
    ),
    _aa_models_benchmark(
        "aa_answer_speed",
        "AA Answer Output Speed",
        "AA Ans Speed",
        "Latency & Cost",
        "Tokens/sec",
        tier=2,
        description="Artificial Analysis median canonical answer output speed.",
    ),
    _aa_models_benchmark(
        "aa_time_to_first_token",
        "AA Time to First Token",
        "AA TTFT",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=2,
        description="Artificial Analysis median time to first token.",
    ),
    _aa_models_benchmark(
        "aa_time_to_first_answer_token",
        "AA Time to First Answer Token",
        "AA TTFAT",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=2,
        description="Artificial Analysis median time to first answer token.",
    ),
    _aa_models_benchmark(
        "aa_end_to_end_response_time",
        "AA End-to-End Response Time",
        "AA E2E",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=2,
        description="Artificial Analysis median end-to-end response time.",
    ),
    _aa_models_benchmark(
        "aa_reasoning_time",
        "AA Reasoning Time",
        "AA Reason",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis median reasoning time when available.",
    ),
    _aa_models_benchmark(
        "aa_speed_p05",
        "AA Output Speed p05",
        "Speed p05",
        "Latency & Cost",
        "Tokens/sec",
        tier=3,
        description="Artificial Analysis fifth-percentile output speed.",
    ),
    _aa_models_benchmark(
        "aa_speed_q25",
        "AA Output Speed q25",
        "Speed q25",
        "Latency & Cost",
        "Tokens/sec",
        tier=3,
        description="Artificial Analysis lower-quartile output speed.",
    ),
    _aa_models_benchmark(
        "aa_speed_q75",
        "AA Output Speed q75",
        "Speed q75",
        "Latency & Cost",
        "Tokens/sec",
        tier=3,
        description="Artificial Analysis upper-quartile output speed.",
    ),
    _aa_models_benchmark(
        "aa_speed_p95",
        "AA Output Speed p95",
        "Speed p95",
        "Latency & Cost",
        "Tokens/sec",
        tier=3,
        description="Artificial Analysis ninety-fifth-percentile output speed.",
    ),
    _aa_models_benchmark(
        "aa_ttft_p05",
        "AA TTFT p05",
        "TTFT p05",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis fifth-percentile time to first token.",
    ),
    _aa_models_benchmark(
        "aa_ttft_q25",
        "AA TTFT q25",
        "TTFT q25",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis lower-quartile time to first token.",
    ),
    _aa_models_benchmark(
        "aa_ttft_q75",
        "AA TTFT q75",
        "TTFT q75",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis upper-quartile time to first token.",
    ),
    _aa_models_benchmark(
        "aa_ttft_p95",
        "AA TTFT p95",
        "TTFT p95",
        "Latency & Cost",
        "Seconds",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis ninety-fifth-percentile time to first token.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_cost_total",
        "AA Intelligence Eval Total Cost",
        "AA Eval $",
        "Latency & Cost",
        "USD",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis total cost to run the Intelligence Index evaluation.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_cost_per_task",
        "AA Intelligence Eval Cost per Task",
        "AA Task $",
        "Latency & Cost",
        "USD/task",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis weighted cost per Intelligence Index evaluation task.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_input_cost",
        "AA Intelligence Eval Input Cost",
        "AA In $",
        "Latency & Cost",
        "USD",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis input-token cost to run the Intelligence Index evaluation.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_output_cost",
        "AA Intelligence Eval Output Cost",
        "AA Out $",
        "Latency & Cost",
        "USD",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis output-token cost to run the Intelligence Index evaluation.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_reasoning_cost",
        "AA Intelligence Eval Reasoning Cost",
        "AA Reason $",
        "Latency & Cost",
        "USD",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis reasoning-token cost to run the Intelligence Index evaluation.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_answer_cost",
        "AA Intelligence Eval Answer Cost",
        "AA Answer $",
        "Latency & Cost",
        "USD",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis answer-token cost to run the Intelligence Index evaluation.",
    ),
    _aa_models_benchmark(
        "aa_intelligence_time_per_task",
        "AA Intelligence Eval Time per Task",
        "AA Task Time",
        "Latency & Cost",
        "Seconds/task",
        higher_is_better=0,
        tier=3,
        description="Artificial Analysis weighted wall-clock time per Intelligence Index evaluation task.",
    ),
]


def provider_id_from_name(name: str) -> str:
    canonical_name = canonical_provider_name(name)
    normalized = re.sub(r"[^a-z0-9]+", "-", canonical_name.strip().lower()).strip("-")
    return normalized or "unknown-provider"


def _clean_optional_text(value: Any) -> str | None:
    if value is None:
        return None
    text_value = str(value).strip()
    return text_value or None


def normalize_origin_countries(
    raw_countries: Iterable[Mapping[str, Any]] | None,
    fallback_country_code: str | None = None,
    fallback_country_name: str | None = None,
) -> list[dict[str, str | None]]:
    normalized: list[dict[str, str | None]] = []
    seen: set[tuple[str | None, str]] = set()

    for item in raw_countries or []:
        raw_code = _clean_optional_text(item.get("code"))
        raw_name = _clean_optional_text(item.get("name"))
        if raw_code is not None:
            raw_code = raw_code.upper()
            if len(raw_code) != 2 or not raw_code.isalpha():
                raise ValueError(f"Invalid provider origin country code: {raw_code}")
        if raw_name is None and raw_code is not None:
            raw_name = raw_code
        if raw_name is None:
            continue
        key = (raw_code, raw_name.casefold())
        if key in seen:
            continue
        seen.add(key)
        normalized.append({"code": raw_code, "name": raw_name})

    if normalized:
        return normalized

    raw_code = _clean_optional_text(fallback_country_code)
    raw_name = _clean_optional_text(fallback_country_name)
    if raw_code is not None:
        raw_code = raw_code.upper()
        if len(raw_code) != 2 or not raw_code.isalpha():
            raise ValueError(f"Invalid provider origin country code: {raw_code}")
    if raw_name is None and raw_code is not None:
        raw_name = raw_code
    if raw_name is None:
        return []
    return [{"code": raw_code, "name": raw_name}]


def derive_provider_origin_fields(origin_countries: Iterable[Mapping[str, Any]] | None) -> tuple[str | None, str | None]:
    countries = normalize_origin_countries(origin_countries)
    if not countries:
        return None, None
    if len(countries) == 1:
        return countries[0].get("code"), countries[0].get("name")
    names = [str(country.get("name") or "").strip() for country in countries if str(country.get("name") or "").strip()]
    return None, ", ".join(dict.fromkeys(names)) or None


def load_provider_origin_baseline(path: Path | None = None) -> list[dict[str, Any]]:
    baseline_path = path or PROVIDER_ORIGIN_BASELINE_PATH
    if not baseline_path.exists():
        return []

    raw_payload = json.loads(baseline_path.read_text(encoding="utf-8"))
    if not isinstance(raw_payload, list):
        raise ValueError("Provider origin baseline must be a JSON array.")

    normalized_payload: list[dict[str, Any]] = []
    for item in raw_payload:
        if not isinstance(item, Mapping):
            continue
        provider_id = _clean_optional_text(item.get("id"))
        provider_name = _clean_optional_text(item.get("name"))
        if provider_id is None or provider_name is None:
            continue
        canonical_name = canonical_provider_name(provider_name)
        if canonical_name != provider_name:
            provider_name = canonical_name
            provider_id = provider_id_from_name(provider_name)
        origin_countries = normalize_origin_countries(
            item.get("origin_countries"),
            _clean_optional_text(item.get("country_code")),
            _clean_optional_text(item.get("country_name")),
        )
        country_code, country_name = derive_provider_origin_fields(origin_countries)
        normalized_payload.append(
            {
                "id": provider_id,
                "name": provider_name,
                "country_code": country_code,
                "country_name": country_name,
                "origin_countries_json": json.dumps(origin_countries, separators=(",", ":")),
                "origin_basis": _clean_optional_text(item.get("origin_basis")),
                "source_url": _clean_optional_text(item.get("source_url")),
                "verified_at": _clean_optional_text(item.get("verified_at")),
                "active": 1 if item.get("active", True) else 0,
            }
        )

    return normalized_payload


def apply_provider_origin_baseline(target: Connection | Engine) -> int:
    if isinstance(target, Engine):
        with target.begin() as conn:
            return apply_provider_origin_baseline(conn)

    rows = load_provider_origin_baseline()
    if not rows:
        return 0

    stmt = sqlite_insert(providers).values(rows)
    update_columns = {
        column.name: getattr(stmt.excluded, column.name)
        for column in providers.columns
        if column.name != "id"
    }
    stmt = stmt.on_conflict_do_update(index_elements=["id"], set_=update_columns)
    target.execute(stmt)
    return len(rows)


def export_provider_origin_baseline(target: Connection | Engine, path: Path | None = None) -> Path:
    baseline_path = path or PROVIDER_ORIGIN_BASELINE_PATH
    if isinstance(target, Engine):
        with target.begin() as conn:
            return export_provider_origin_baseline(conn, baseline_path)

    rows = (
        target.execute(select(providers).where(providers.c.active == 1).order_by(providers.c.name.asc()))
        .mappings()
        .all()
    )
    payload: list[dict[str, Any]] = []
    for row in rows:
        origin_countries = normalize_origin_countries(
            json.loads(str(row.get("origin_countries_json") or "[]")),
            row.get("country_code"),
            row.get("country_name"),
        )
        payload.append(
            {
                "id": str(row["id"]),
                "name": str(row["name"]),
                "origin_countries": origin_countries,
                "origin_basis": _clean_optional_text(row.get("origin_basis")),
                "source_url": _clean_optional_text(row.get("source_url")),
                "verified_at": _clean_optional_text(row.get("verified_at")),
                "active": bool(row.get("active", 1)),
            }
        )

    baseline_path.write_text(json.dumps(payload, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    return baseline_path

BENCHMARKS: list[dict[str, Any]] = [
    {
        "id": INTERNAL_VIEW_BENCHMARK_ID,
        "name": "Internal View",
        "short": "Internal",
        "source": "Internal",
        "url": "",
        "category": "Internal Assessment",
        "metric": "Internal Score",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "InternalManualBenchmark",
        "description": "Optional internal assessment signal entered manually to reflect business context, rollout preferences, or operator experience.",
        "active": 1,
    },
    {
        "id": "aa_intelligence",
        "name": "AA Intelligence Index",
        "short": "AA Intel",
        "source": "Artificial Analysis",
        "url": AA_MODELS_URL,
        "category": "General Quality",
        "metric": "Index Score",
        "higher_is_better": 1,
        "tier": 1,
        "scraper_id": "ArtificialAnalysisAdapter",
        "description": "Composite quality index across multiple tasks. Best single number for overall capability.",
        "active": 1,
    },
    *AA_MODELS_EXTRA_BENCHMARKS,
    {
        "id": "aa_ifbench",
        "name": "AA IFBench",
        "short": "AA IFBench",
        "source": "Artificial Analysis",
        "url": "https://artificialanalysis.ai/evaluations/ifbench",
        "category": "Instruction Following",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "ArtificialAnalysisIfbenchAdapter",
        "description": "Artificial Analysis IFBench score for instruction following. Parsed from the public evaluation page JSON-LD dataset and the models leaderboard payload when available.",
        "active": 1,
    },
    {
        "id": "aa_ifbench_output_tokens",
        "name": "AA IFBench Output Tokens",
        "short": "IF Tokens",
        "source": "Artificial Analysis",
        "url": "https://artificialanalysis.ai/evaluations/ifbench",
        "category": "Latency & Cost",
        "metric": "Output tokens/task",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "ArtificialAnalysisIfbenchAdapter",
        "description": "Answer plus reasoning output tokens per IFBench task. Companion efficiency evidence; lower is generally cheaper and faster for equivalent quality.",
        "active": 1,
    },
    {
        "id": "aa_ifbench_cost",
        "name": "AA IFBench Cost",
        "short": "IF Cost",
        "source": "Artificial Analysis",
        "url": "https://artificialanalysis.ai/evaluations/ifbench",
        "category": "Latency & Cost",
        "metric": "USD/task",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "ArtificialAnalysisIfbenchAdapter",
        "description": "Estimated total model cost per IFBench task from Artificial Analysis cost components.",
        "active": 1,
    },
    {
        "id": "aa_ifbench_time",
        "name": "AA IFBench Time",
        "short": "IF Time",
        "source": "Artificial Analysis",
        "url": "https://artificialanalysis.ai/evaluations/ifbench",
        "category": "Latency & Cost",
        "metric": "Minutes/task",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "ArtificialAnalysisIfbenchAdapter",
        "description": "Weighted average wall-clock minutes per IFBench task, excluding TTFT and execution time where Artificial Analysis documents that boundary.",
        "active": 1,
    },
    {
        "id": "gpqa_diamond",
        "name": "GPQA Diamond",
        "short": "GPQA-D",
        "source": "Epoch AI",
        "url": "https://epoch.ai/benchmarks/gpqa-diamond",
        "category": "Reasoning & Math",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 1,
        "scraper_id": "EpochGpqaAdapter",
        "description": "Graduate-level science questions. Highly resistant to contamination.",
        "active": 1,
    },
    {
        "id": "mmmu",
        "name": "MMMU",
        "short": "MMMU",
        "source": "MMMU team",
        "url": "https://mmmu-benchmark.github.io/",
        "category": "Multimodal",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "MmmuAdapter",
        "description": "Useful academic multimodal signal, but the public leaderboard updates slowly and this ingestion uses the validation split rather than a fresh held-out test feed.",
        "active": 1,
    },
    {
        "id": "mmmu_test",
        "name": "MMMU Test",
        "short": "MMMU Test",
        "source": "MMMU team",
        "url": "https://mmmu-benchmark.github.io/",
        "category": "Multimodal",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "MmmuAdapter",
        "description": "MMMU held-out test overall score when the upstream leaderboard publishes stable model-level test results. Human/random baselines are skipped.",
        "active": 1,
    },
    {
        "id": "mmmu_pro",
        "name": "MMMU-Pro",
        "short": "MMMU-Pro",
        "source": "MMMU team",
        "url": "https://mmmu-benchmark.github.io/",
        "category": "Multimodal",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "MmmuAdapter",
        "description": "MMMU-Pro overall score when the upstream leaderboard publishes stable model-level pro results. Validation behavior remains unchanged, and human/random baselines are skipped.",
        "active": 1,
    },
    {
        "id": "swebench_verified",
        "name": "SWE-bench Verified",
        "short": "SWE-V",
        "source": "SWE-bench team",
        "url": "https://www.swebench.com/",
        "category": "Coding",
        "metric": "% Issues Resolved",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "SwebenchAdapter",
        "description": "Derived from the official SWE-bench Verified board using the best single-model system submission per model. Strong coding evidence, but still agent-derived rather than a pure model-only eval.",
        "active": 1,
    },
    {
        "id": "swebench_full",
        "name": "SWE-bench Full",
        "short": "SWE-F",
        "source": "SWE-bench team",
        "url": "https://www.swebench.com/#full",
        "category": "Coding",
        "metric": "% Issues Resolved",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "SwebenchAdapter",
        "description": "Derived from the official SWE-bench Full/Test board using the best single-model system submission per model. Companion split evidence; ranking weights still use Verified first.",
        "active": 1,
    },
    {
        "id": "swebench_lite",
        "name": "SWE-bench Lite",
        "short": "SWE-L",
        "source": "SWE-bench team",
        "url": "https://www.swebench.com/#lite",
        "category": "Coding",
        "metric": "% Issues Resolved",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "SwebenchAdapter",
        "description": "Derived from the official SWE-bench Lite board using the best single-model system submission per model. Companion split evidence for smaller issue sets.",
        "active": 1,
    },
    {
        "id": "swebench_multilingual",
        "name": "SWE-bench Multilingual",
        "short": "SWE-ML",
        "source": "SWE-bench team",
        "url": "https://www.swebench.com/#multilingual",
        "category": "Coding",
        "metric": "% Issues Resolved",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "SwebenchAdapter",
        "description": "Derived from the official SWE-bench Multilingual board using the best single-model system submission per model.",
        "active": 1,
    },
    {
        "id": "swebench_multimodal",
        "name": "SWE-bench Multimodal",
        "short": "SWE-MM",
        "source": "SWE-bench team",
        "url": "https://www.swebench.com/#multimodal",
        "category": "Coding",
        "metric": "% Issues Resolved",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "SwebenchAdapter",
        "description": "Derived from the official SWE-bench Multimodal board using the best single-model system submission per model.",
        "active": 1,
    },
    {
        "id": "livecodebench_codegen",
        "name": "LiveCodeBench Code Generation",
        "short": "LCB Code",
        "source": "LiveCodeBench",
        "url": "https://livecodebench.github.io/leaderboard.html",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveCodeBenchAdapter",
        "description": "Code-generation pass@1 aggregated from the official LiveCodeBench default date window. The adapter preserves window, difficulty, platform, release-date, and contamination metadata for review.",
        "active": 1,
    },
    {
        "id": "ifeval",
        "name": "IFEval",
        "short": "IFEval",
        "source": "LLM Stats / ZeroEval",
        "url": "https://llm-stats.com/benchmarks/ifeval",
        "category": "Instruction Following",
        "metric": "Prompt Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "IfevalAdapter",
        "description": "Instruction-following signal pulled from a live feed that is currently overwhelmingly self-reported and unverified. Useful context, but lower-trust than primary benchmark publishers.",
        "active": 1,
    },
    {
        "id": "rag_groundedness",
        "name": "Vectara Hallucination Leaderboard",
        "short": "Vectara FC",
        "source": "Vectara",
        "url": "https://github.com/vectara/hallucination-leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Factual Consistency %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "VectaraHallucinationAdapter",
        "description": "Grounded summarization faithfulness to supplied source text. Useful directional evidence, but narrower than end-to-end retrieval relevance or enterprise RAG quality.",
        "active": 1,
    },
    {
        "id": "rag_hallucination_rate",
        "name": "Vectara Hallucination Rate",
        "short": "Vectara Halluc.",
        "source": "Vectara",
        "url": "https://github.com/vectara/hallucination-leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "VectaraHallucinationAdapter",
        "description": "Grounded summarization hallucination rate from the Vectara leaderboard. Lower is better; this remains a faithfulness signal rather than retrieval relevance.",
        "active": 1,
    },
    {
        "id": "rag_answer_rate",
        "name": "Vectara Answer Rate",
        "short": "Vectara Answer",
        "source": "Vectara",
        "url": "https://github.com/vectara/hallucination-leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Answer Rate %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "VectaraHallucinationAdapter",
        "description": "Answer-rate companion metric from the Vectara hallucination leaderboard. Useful coverage context for grounded summarization, not a retrieval-quality score.",
        "active": 1,
    },
    {
        "id": "rag_task_faithfulness",
        "name": "FaithJudge",
        "short": "FaithJudge",
        "source": "Vectara",
        "url": "https://github.com/vectara/FaithJudge#leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "FaithJudgeAdapter",
        "description": "Small vendor-run hallucination benchmark across FaithBench and RagTruth tasks with supplied context. Useful directional signal, but coverage is narrow and not a retrieval relevance eval.",
        "active": 1,
    },
    {
        "id": "faithjudge_faithbench_summarization",
        "name": "FaithJudge FaithBench Summarization",
        "short": "FJ FaithBench",
        "source": "Vectara",
        "url": "https://github.com/vectara/FaithJudge#leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "FaithJudgeAdapter",
        "description": "FaithJudge FaithBench summarization hallucination-rate subtask. Lower is better; useful task-level faithfulness context, not retrieval relevance.",
        "active": 1,
    },
    {
        "id": "faithjudge_ragtruth_summarization",
        "name": "FaithJudge RAGTruth Summarization",
        "short": "FJ RT Sum",
        "source": "Vectara",
        "url": "https://github.com/vectara/FaithJudge#leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "FaithJudgeAdapter",
        "description": "FaithJudge RAGTruth summarization hallucination-rate subtask. Lower is better; useful task-level faithfulness context, not retrieval relevance.",
        "active": 1,
    },
    {
        "id": "faithjudge_ragtruth_question_answering",
        "name": "FaithJudge RAGTruth QA",
        "short": "FJ RT QA",
        "source": "Vectara",
        "url": "https://github.com/vectara/FaithJudge#leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "FaithJudgeAdapter",
        "description": "FaithJudge RAGTruth question-answering hallucination-rate subtask. Lower is better; useful task-level faithfulness context, not retrieval relevance.",
        "active": 1,
    },
    {
        "id": "faithjudge_ragtruth_data_to_text",
        "name": "FaithJudge RAGTruth Data-to-Text",
        "short": "FJ RT D2T",
        "source": "Vectara",
        "url": "https://github.com/vectara/FaithJudge#leaderboard",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "FaithJudgeAdapter",
        "description": "FaithJudge RAGTruth data-to-text hallucination-rate subtask. Lower is better; useful task-level faithfulness context, not retrieval relevance.",
        "active": 1,
    },
    {
        "id": "mteb_retrieval",
        "name": "MTEB Retrieval",
        "short": "MTEB Ret.",
        "source": "MTEB",
        "url": "https://github.com/embeddings-benchmark/results",
        "category": "Retrieval & Grounding",
        "metric": "Average main score %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "MtebAdapter",
        "description": "Average MTEB retrieval task main score for embedding models, sourced from the official embeddings-benchmark/results repository. Used only in embedding/reranking model-role use cases.",
        "active": 1,
    },
    {
        "id": "mteb_reranking",
        "name": "MTEB Reranking",
        "short": "MTEB Rerank",
        "source": "MTEB",
        "url": "https://github.com/embeddings-benchmark/results",
        "category": "Retrieval & Grounding",
        "metric": "Average main score %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "MtebAdapter",
        "description": "Average MTEB reranking task main score for reranker-capable models, sourced from the official embeddings-benchmark/results repository. Used only in reranker model-role use cases.",
        "active": 1,
    },
    {
        "id": "mteb_retrieval_reranking",
        "name": "MTEB Retrieval/Reranking",
        "short": "MTEB RAG",
        "source": "MTEB",
        "url": "https://github.com/embeddings-benchmark/results",
        "category": "Retrieval & Grounding",
        "metric": "Average main score %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "MtebAdapter",
        "description": "Combined MTEB retrieval and reranking average for RAG retrieval-sorting model selection. It is intentionally separate from generator-model RAG answer-quality use cases.",
        "active": 1,
    },
    {
        "id": "rteb_finance",
        "name": "RTEB Finance",
        "short": "RTEB Fin.",
        "source": "MTEB",
        "url": "https://huggingface.co/spaces/mteb/leaderboard?benchmark_name=RTEB%28fin%2C%20beta%29",
        "category": "Retrieval & Grounding",
        "metric": "Average NDCG@10 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "MtebAdapter",
        "description": "RTEB(fin, beta) retrieval quality for financial-domain embedding selection, aggregated from the official mteb/results dataset across finance benchmarks, Q&A, financial document retrieval, corporate-governance, and closed/private finance retrieval tasks.",
        "active": 1,
    },
    {
        "id": "terminal_bench",
        "name": "Terminal-Bench 2.0",
        "short": "Term-B",
        "source": "tbench.ai",
        "url": "https://www.tbench.ai/leaderboard/terminal-bench/2.0",
        "category": "Agentic & Tool Use",
        "metric": "% Tasks Resolved",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TerminalBenchAdapter",
        "description": "Real workflow terminal tasks. Current policy derives one score per model from the best verified single-model submission.",
        "active": 1,
    },
    {
        "id": "ailuminate",
        "name": "AILuminate",
        "short": "Safety",
        "source": "MLCommons",
        "url": "https://ailuminate.mlcommons.org/benchmarks/",
        "category": "Safety & Compliance",
        "metric": "Grade (0-100)",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "AILuminateAdapter",
        "description": "Official MLCommons safety benchmark, but the public named results are coarse grade bands and mix bare models with broader AI systems.",
        "active": 1,
    },
    {
        "id": "ailuminate_en_us",
        "name": "AILuminate en-US",
        "short": "Safety en",
        "source": "MLCommons",
        "url": "https://ailuminate.mlcommons.org/benchmarks/general_purpose_ai_chat/1.0-en_us-official-ensemble",
        "category": "Safety & Compliance",
        "metric": "Grade (0-100)",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "AILuminateAdapter",
        "description": "AILuminate public grade restricted to the en_us locale surface. Companion evidence only; current safety rankings keep the aggregate grade.",
        "active": 1,
    },
    {
        "id": "ailuminate_fr_fr",
        "name": "AILuminate fr-FR",
        "short": "Safety fr",
        "source": "MLCommons",
        "url": "https://ailuminate.mlcommons.org/benchmarks/general_purpose_ai_chat/1.0-fr_fr-official-ensemble",
        "category": "Safety & Compliance",
        "metric": "Grade (0-100)",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "AILuminateAdapter",
        "description": "AILuminate public grade restricted to the fr_fr locale surface. Companion evidence only; coverage is currently much smaller than en_us.",
        "active": 1,
    },
    {
        "id": "ailuminate_ai_systems",
        "name": "AILuminate AI Systems",
        "short": "Safety Sys",
        "source": "MLCommons",
        "url": "https://ailuminate.mlcommons.org/benchmarks/",
        "category": "Safety & Compliance",
        "metric": "Grade (0-100)",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "AILuminateAdapter",
        "description": "AILuminate public grade restricted to the AI Systems table, preserving moderated/system-level evidence separately from bare model evidence.",
        "active": 1,
    },
    {
        "id": "ailuminate_bare_models",
        "name": "AILuminate Bare Models",
        "short": "Safety Bare",
        "source": "MLCommons",
        "url": "https://ailuminate.mlcommons.org/benchmarks/",
        "category": "Safety & Compliance",
        "metric": "Grade (0-100)",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "AILuminateAdapter",
        "description": "AILuminate public grade restricted to the Bare Models table, preserving base-model safety evidence separately from system-level results.",
        "active": 1,
    },
    {
        "id": "aa_speed",
        "name": "Output Speed",
        "short": "Speed",
        "source": "Artificial Analysis",
        "url": AA_MODELS_URL,
        "category": "Latency & Cost",
        "metric": "Tokens/sec",
        "higher_is_better": 1,
        "tier": 1,
        "scraper_id": "ArtificialAnalysisAdapter",
        "description": "Output tokens per second. Critical for real-time and high-concurrency systems.",
        "active": 1,
    },
    {
        "id": "aa_cost",
        "name": "Cost (blended)",
        "short": "Cost",
        "source": "Artificial Analysis",
        "url": AA_MODELS_URL,
        "category": "Latency & Cost",
        "metric": "$/1M tokens",
        "higher_is_better": 0,
        "tier": 1,
        "scraper_id": "ArtificialAnalysisAdapter",
        "description": "Artificial Analysis default blended 3:1 input-output pricing. Lower is better.",
        "active": 1,
    },
    {
        "id": "chatbot_arena",
        "name": "Chatbot Arena",
        "short": "Arena ELO",
        "source": "LMSYS",
        "url": "https://arena.ai/leaderboard/text",
        "category": "General Quality",
        "metric": "ELO Score",
        "higher_is_better": 1,
        "tier": 1,
        "scraper_id": "ChatbotArenaAdapter",
        "description": "ELO from blind human preference votes.",
        "active": 1,
    },
    {
        "id": "livebench_overall",
        "name": "LiveBench Overall",
        "short": "LiveBench",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "General Quality",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Global average from official LiveBench public static leaderboard category averages for the latest publicly released question set.",
        "active": 1,
    },
    {
        "id": "livebench_reasoning",
        "name": "LiveBench Reasoning",
        "short": "LB Reason",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Reasoning & Math",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Reasoning category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_coding",
        "name": "LiveBench Coding",
        "short": "LB Code",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Coding",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Coding category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_agentic_coding",
        "name": "LiveBench Agentic Coding",
        "short": "LB Agent Code",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Agentic & Tool Use",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Agentic coding category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_math",
        "name": "LiveBench Mathematics",
        "short": "LB Math",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Reasoning & Math",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Mathematics category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_data_analysis",
        "name": "LiveBench Data Analysis",
        "short": "LB Data",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "General Quality",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Data analysis category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_language",
        "name": "LiveBench Language",
        "short": "LB Lang",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Instruction Following",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Language category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "livebench_instruction_following",
        "name": "LiveBench Instruction Following",
        "short": "LB IF",
        "source": "LiveBench",
        "url": "https://livebench.ai/",
        "category": "Instruction Following",
        "metric": "Category Average %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "LiveBenchAdapter",
        "description": "Instruction-following category average from official LiveBench public static leaderboard task scores.",
        "active": 1,
    },
    {
        "id": "bfcl_overall",
        "name": "Berkeley Function Calling Leaderboard",
        "short": "BFCL",
        "source": "Berkeley Gorilla",
        "url": "https://gorilla.cs.berkeley.edu/leaderboard.html",
        "category": "Agentic & Tool Use",
        "metric": "Overall Accuracy %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "BfclAdapter",
        "description": "Official Berkeley Function Calling Leaderboard V4 overall accuracy for function invocation and tool-use behavior. Component scores, costs, latency, and evaluation mode are preserved as source metadata.",
        "active": 1,
    },
    {
        "id": "bigcodebench_full",
        "name": "BigCodeBench Full",
        "short": "BCB-F",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Full Set average of instruct and complete Pass@1 scores using greedy decoding.",
        "active": 1,
    },
    {
        "id": "bigcodebench_full_instruct",
        "name": "BigCodeBench Full Instruct",
        "short": "BCB-F-I",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Full Set instruct Pass@1 score.",
        "active": 1,
    },
    {
        "id": "bigcodebench_full_complete",
        "name": "BigCodeBench Full Complete",
        "short": "BCB-F-C",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Full Set complete Pass@1 score.",
        "active": 1,
    },
    {
        "id": "bigcodebench_hard",
        "name": "BigCodeBench Hard",
        "short": "BCB-H",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Hard subset average of instruct and complete Pass@1 scores.",
        "active": 1,
    },
    {
        "id": "bigcodebench_hard_instruct",
        "name": "BigCodeBench Hard Instruct",
        "short": "BCB-H-I",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Hard subset instruct Pass@1 score.",
        "active": 1,
    },
    {
        "id": "bigcodebench_hard_complete",
        "name": "BigCodeBench Hard Complete",
        "short": "BCB-H-C",
        "source": "BigCodeBench",
        "url": "https://bigcode-bench.github.io/",
        "category": "Coding",
        "metric": "Pass@1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "BigCodeBenchAdapter",
        "description": "Official BigCodeBench Hard subset complete Pass@1 score.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_mean",
        "name": "HELM Capabilities Mean",
        "short": "HELM-C",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "General Quality",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities core-scenarios mean accuracy across MMLU-Pro, GPQA, IFEval, WildBench, and Omni-MATH.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_mmlu_pro",
        "name": "HELM Capabilities MMLU-Pro",
        "short": "HELM-MMLU",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "Reasoning & Math",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities MMLU-Pro chain-of-thought correctness component.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_gpqa",
        "name": "HELM Capabilities GPQA",
        "short": "HELM-GPQA",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "Reasoning & Math",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities GPQA chain-of-thought correctness component.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_ifeval",
        "name": "HELM Capabilities IFEval",
        "short": "HELM-IFE",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "Instruction Following",
        "metric": "Strict Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities IFEval strict accuracy component.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_wildbench",
        "name": "HELM Capabilities WildBench",
        "short": "HELM-WB",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "General Quality",
        "metric": "Score %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities WildBench score component.",
        "active": 1,
    },
    {
        "id": "helm_capabilities_omni_math",
        "name": "HELM Capabilities Omni-MATH",
        "short": "HELM-OM",
        "source": "Stanford CRFM",
        "url": "https://crfm.stanford.edu/helm/capabilities/latest/",
        "category": "Reasoning & Math",
        "metric": "Accuracy %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "HelmCapabilitiesAdapter",
        "description": "Official HELM Capabilities Omni-MATH accuracy component.",
        "active": 1,
    },
    {
        "id": "taubench_text_mean",
        "name": "tau-bench Text Mean",
        "short": "tau-text",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TaubenchAdapter",
        "description": "Mean pass^1 score across the current complete text tau-bench domain set. Secondary agent-system evidence, not a pure model-only eval.",
        "active": 1,
    },
    {
        "id": "taubench_text_airline",
        "name": "tau-bench Text Airline",
        "short": "tau-air",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TaubenchAdapter",
        "description": "Text tau-bench airline-domain pass^1 score from current standard single-model submissions.",
        "active": 1,
    },
    {
        "id": "taubench_text_retail",
        "name": "tau-bench Text Retail",
        "short": "tau-ret",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TaubenchAdapter",
        "description": "Text tau-bench retail-domain pass^1 score from current standard single-model submissions.",
        "active": 1,
    },
    {
        "id": "taubench_text_telecom",
        "name": "tau-bench Text Telecom",
        "short": "tau-tel",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TaubenchAdapter",
        "description": "Text tau-bench telecom-domain pass^1 score from current standard single-model submissions.",
        "active": 1,
    },
    {
        "id": "taubench_text_banking_knowledge",
        "name": "tau-bench Text Banking Knowledge",
        "short": "tau-bank",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Retrieval & Grounding",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 2,
        "scraper_id": "TaubenchAdapter",
        "description": "Text tau-bench banking knowledge-domain pass^1 score, including retrieval configuration metadata when provided.",
        "active": 1,
    },
    {
        "id": "taubench_voice_mean",
        "name": "tau-bench Voice Mean",
        "short": "tau-voice",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "TaubenchAdapter",
        "description": "Mean pass^1 score across the current complete voice tau-bench domain set. Secondary audio-agent evidence.",
        "active": 1,
    },
    {
        "id": "taubench_voice_airline",
        "name": "tau-bench Voice Airline",
        "short": "tau-vair",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "TaubenchAdapter",
        "description": "Voice tau-bench airline-domain pass^1 score from current standard single-model audio submissions.",
        "active": 1,
    },
    {
        "id": "taubench_voice_retail",
        "name": "tau-bench Voice Retail",
        "short": "tau-vret",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "TaubenchAdapter",
        "description": "Voice tau-bench retail-domain pass^1 score from current standard single-model audio submissions.",
        "active": 1,
    },
    {
        "id": "taubench_voice_telecom",
        "name": "tau-bench Voice Telecom",
        "short": "tau-vtel",
        "source": "Sierra",
        "url": "https://taubench.com/",
        "category": "Agentic & Tool Use",
        "metric": "Pass^1 %",
        "higher_is_better": 1,
        "tier": 3,
        "scraper_id": "TaubenchAdapter",
        "description": "Voice tau-bench telecom-domain pass^1 score from current standard single-model audio submissions.",
        "active": 1,
    },
    {
        "id": "ragtruth_hallucination_rate",
        "name": "RAGTruth Hallucination Rate",
        "short": "RAGTruth",
        "source": "Particle Media",
        "url": "https://github.com/ParticleMedia/RAGTruth",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "RagtruthAdapter",
        "description": "Official RAGTruth held-out test split response-level hallucination rate across Summary, QA, and Data-to-text tasks. Lower is better; coverage is historical.",
        "active": 1,
    },
    {
        "id": "ragtruth_summary_hallucination_rate",
        "name": "RAGTruth Summarization Hallucination Rate",
        "short": "RAGTruth-Sum",
        "source": "Particle Media",
        "url": "https://github.com/ParticleMedia/RAGTruth",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "RagtruthAdapter",
        "description": "Official RAGTruth held-out summarization response-level hallucination rate. Lower is better.",
        "active": 1,
    },
    {
        "id": "ragtruth_qa_hallucination_rate",
        "name": "RAGTruth QA Hallucination Rate",
        "short": "RAGTruth-QA",
        "source": "Particle Media",
        "url": "https://github.com/ParticleMedia/RAGTruth",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "RagtruthAdapter",
        "description": "Official RAGTruth held-out question-answering response-level hallucination rate. Lower is better.",
        "active": 1,
    },
    {
        "id": "ragtruth_data_to_text_hallucination_rate",
        "name": "RAGTruth Data-to-text Hallucination Rate",
        "short": "RAGTruth-D2T",
        "source": "Particle Media",
        "url": "https://github.com/ParticleMedia/RAGTruth",
        "category": "Retrieval & Grounding",
        "metric": "Hallucination Rate %",
        "higher_is_better": 0,
        "tier": 3,
        "scraper_id": "RagtruthAdapter",
        "description": "Official RAGTruth held-out data-to-text response-level hallucination rate. Lower is better.",
        "active": 1,
    },
]

PROVIDERS: list[dict[str, Any]] = [
    {
        "id": provider_id_from_name("OpenAI"),
        "name": "OpenAI",
        "country_code": "US",
        "country_name": "United States",
        "origin_basis": "Official OpenAI document listing San Francisco, California.",
        "source_url": "https://cdn.openai.com/global-affairs/openai-doe-rfi-5-7-2025.pdf",
        "verified_at": PROVIDER_ORIGIN_VERIFIED_AT,
        "active": 1,
    },
    {
        "id": provider_id_from_name("Anthropic"),
        "name": "Anthropic",
        "country_code": "US",
        "country_name": "United States",
        "origin_basis": "Official Anthropic privacy notice listing San Francisco, California.",
        "source_url": "https://www-cdn.anthropic.com/8fd65f023a39d74220c5ebff85c788875c05533f.pdf",
        "verified_at": PROVIDER_ORIGIN_VERIFIED_AT,
        "active": 1,
    },
    {
        "id": provider_id_from_name("Google"),
        "name": "Google",
        "country_code": "US",
        "country_name": "United States",
        "origin_basis": "Official Google company information and locations pages.",
        "source_url": "https://about.google/company-info/locations/",
        "verified_at": PROVIDER_ORIGIN_VERIFIED_AT,
        "active": 1,
    },
    {
        "id": provider_id_from_name("Inception Labs"),
        "name": "Inception Labs",
        "country_code": "US",
        "country_name": "United States",
        "origin_basis": "Official Inception Labs legal documentation for the company.",
        "source_url": "https://www.inceptionlabs.ai/docs/terms-of-use",
        "verified_at": PROVIDER_ORIGIN_VERIFIED_AT,
        "active": 1,
    },
    {
        "id": provider_id_from_name("Zhipu AI"),
        "name": "Zhipu AI",
        "country_code": "CN",
        "country_name": "China",
        "origin_basis": "Official Zhipu agreement naming the Beijing legal entity.",
        "source_url": "https://docs.bigmodel.cn/cn/terms/recharge-agreement",
        "verified_at": PROVIDER_ORIGIN_VERIFIED_AT,
        "active": 1,
    },
]

MODELS: list[dict[str, Any]] = [
    {
        "id": "gemini-3-1-pro",
        "name": "Gemini 3.1 Pro Preview",
        "provider": "Google",
        "type": "proprietary",
        "release_date": "2026-Q1",
        "context_window": "1M tokens",
        "active": 1,
    },
    {
        "id": "gpt-5-4",
        "name": "GPT-5.4 (xhigh)",
        "provider": "OpenAI",
        "type": "proprietary",
        "release_date": "2026-Q1",
        "context_window": "128k tokens",
        "active": 1,
    },
    {
        "id": "gpt-5-3-codex",
        "name": "GPT-5.3 Codex (xhigh)",
        "provider": "OpenAI",
        "type": "proprietary",
        "release_date": "2026-Q1",
        "context_window": "128k tokens",
        "active": 1,
    },
    {
        "id": "claude-opus-4-6",
        "name": "Claude Opus 4.6",
        "provider": "Anthropic",
        "type": "proprietary",
        "release_date": "2025",
        "context_window": "200k tokens",
        "active": 1,
    },
    {
        "id": "claude-sonnet-4-6",
        "name": "Claude Sonnet 4.6",
        "provider": "Anthropic",
        "type": "proprietary",
        "release_date": "2025",
        "context_window": "200k tokens",
        "active": 1,
    },
    {
        "id": "o4-mini-high",
        "name": "o4 Mini High",
        "provider": "OpenAI",
        "type": "proprietary",
        "release_date": "2025",
        "context_window": "128k tokens",
        "active": 1,
    },
    {
        "id": "glm-5",
        "name": "GLM-5 (Reasoning)",
        "provider": "Zhipu AI",
        "type": "open_weights",
        "release_date": "2026-Q1",
        "context_window": "128k tokens",
        "active": 1,
    },
    {
        "id": "gemini-2-5-flash",
        "name": "Gemini 2.5 Flash-Lite (Reasoning)",
        "provider": "Google",
        "type": "proprietary",
        "release_date": "2025",
        "context_window": "1M tokens",
        "active": 1,
    },
    {
        "id": "mercury-2",
        "name": "Mercury 2",
        "provider": "Inception Labs",
        "type": "proprietary",
        "release_date": "2026-Q1",
        "context_window": "32k tokens",
        "active": 1,
    },
    {
        "id": "gemma-3n-e4b",
        "name": "Gemma 3n E4B Instruct",
        "provider": "Google",
        "type": "open_weights",
        "release_date": "2026-Q1",
        "context_window": "8k tokens",
        "active": 1,
    },
]

USE_CASES: list[dict[str, Any]] = [
    {
        "id": "general_reasoning",
        "label": "General Reasoning",
        "icon": "🧠",
        "description": "Logic, multi-step problem solving, knowledge-intensive tasks",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": ["gpqa_diamond"],
        "benchmark_notes": {
            "gpqa_diamond": "Hard factual reasoning and exam-style problem solving.",
            "chatbot_arena": "User-facing helpfulness and response quality context.",
            "aa_intelligence": "Broad capability proxy for general model strength.",
        },
        "weights": {"aa_intelligence": 0.40, "gpqa_diamond": 0.35, "chatbot_arena": 0.25},
    },
    {
        "id": "coding",
        "label": "Coding & Engineering",
        "icon": "💻",
        "description": "Code generation, debugging, repo-level tasks, DevOps",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.45,
        "required_benchmarks": ["swebench_verified", "terminal_bench"],
        "benchmark_notes": {
            "swebench_verified": "Repo-level bug fixing and code-change execution.",
            "terminal_bench": "Agent-derived workflow evidence from verified single-model submissions.",
            "aa_intelligence": "General capability proxy for coding-adjacent reasoning.",
        },
        "weights": {"swebench_verified": 0.55, "aa_intelligence": 0.25, "terminal_bench": 0.20},
    },
    {
        "id": "agentic",
        "label": "Agentic & Tool Use",
        "icon": "🤖",
        "description": "Multi-step workflows, API calls, terminal and browser automation",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": ["terminal_bench", "swebench_verified"],
        "benchmark_notes": {
            "terminal_bench": "Agent-derived workflow evidence from verified single-model submissions.",
            "swebench_verified": "Tool-using repo-level execution under realistic constraints.",
            "aa_intelligence": "General capability proxy for planning and reasoning.",
        },
        "weights": {"terminal_bench": 0.50, "swebench_verified": 0.25, "aa_intelligence": 0.25},
    },
    {
        "id": "safety_compliance",
        "label": "Safety & Compliance",
        "icon": "🛡️",
        "description": "Harmful output avoidance, hazard categories, regulatory readiness",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": [],
        "benchmark_notes": {
            "ailuminate": "Low-confidence safety context from coarse public MLCommons grades.",
            "aa_intelligence": "Primary fallback capability context while safety evidence remains thin.",
        },
        "weights": {"aa_intelligence": 0.80, "ailuminate": 0.20},
    },
    {
        "id": "cost_efficiency",
        "label": "Cost Efficiency",
        "icon": "💰",
        "description": "Minimise spend while maintaining quality for high-volume use",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": ["aa_cost", "aa_speed"],
        "benchmark_notes": {
            "aa_cost": "Unit economics and spend per generated output.",
            "aa_speed": "Throughput and latency under load.",
            "aa_intelligence": "Quality context so low cost does not hide weak capability.",
        },
        "weights": {"aa_cost": 0.60, "aa_speed": 0.25, "aa_intelligence": 0.15},
    },
    {
        "id": "multimodal",
        "label": "Multimodal",
        "icon": "🖼️",
        "description": "Vision-language, chart understanding, document image processing",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": [],
        "benchmark_notes": {
            "mmmu": "Low-confidence multimodal context from a slowly updated academic leaderboard.",
            "aa_intelligence": "Primary capability anchor until stronger multimodal evidence is added.",
        },
        "weights": {"aa_intelligence": 0.85, "mmmu": 0.15},
    },
    {
        "id": "instruction_following",
        "label": "Instruction Following",
        "icon": "📋",
        "description": "Strict adherence to constraints, formatting rules, complex prompt structures",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": [],
        "benchmark_notes": {
            "ifeval": "Low-confidence context from a feed dominated by self-reported scores.",
            "chatbot_arena": "Response quality context.",
            "aa_intelligence": "Primary fallback for overall instruction-heavy task competence.",
        },
        "weights": {"aa_intelligence": 0.55, "chatbot_arena": 0.30, "ifeval": 0.15},
    },
    {
        "id": "speed",
        "label": "Speed / Latency",
        "icon": "⚡",
        "description": "Maximum throughput for real-time, streaming, or high-concurrency apps",
        "segment": "core",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": ["aa_speed"],
        "benchmark_notes": {
            "aa_speed": "Throughput and latency under production-like load.",
            "aa_intelligence": "Quality context so the fastest model is not selected blindly.",
        },
        "weights": {"aa_speed": 0.80, "aa_intelligence": 0.20},
    },
    {
        "id": "enterprise_automation",
        "label": "Enterprise Automation",
        "icon": "🏢",
        "description": "Internal copilots, workflow execution, ticket triage, and system actions under enterprise constraints",
        "segment": "enterprise",
        "status": "preview",
        "min_coverage": 0.55,
        "required_benchmarks": ["terminal_bench"],
        "benchmark_notes": {
            "terminal_bench": "Agent-derived workflow evidence from verified single-model submissions.",
            "ifeval": "Low-confidence instruction-following context only.",
            "ailuminate": "Low-confidence safety context only.",
            "aa_intelligence": "General capability context.",
            "aa_cost": "Operational spend for high-volume automation.",
            "aa_speed": "Latency for interactive workflows.",
        },
        "weights": {
            "terminal_bench": 0.50,
            "aa_intelligence": 0.25,
            "aa_cost": 0.10,
            "aa_speed": 0.05,
            "ifeval": 0.05,
            "ailuminate": 0.05,
        },
    },
    {
        "id": "customer_support",
        "label": "Customer Support",
        "icon": "🎧",
        "description": "Service desk chat, escalation handling, response quality, safety, and operating cost at scale",
        "segment": "enterprise",
        "status": "preview",
        "min_coverage": 0.5,
        "required_benchmarks": ["chatbot_arena"],
        "benchmark_notes": {
            "chatbot_arena": "Preference and conversational quality for support-style interactions.",
            "ifeval": "Low-confidence policy/format-following context only.",
            "ailuminate": "Low-confidence public safety context only.",
            "aa_cost": "Unit economics for support at scale.",
            "aa_speed": "Latency for interactive service conversations.",
        },
        "weights": {
            "chatbot_arena": 0.55,
            "aa_cost": 0.20,
            "aa_speed": 0.10,
            "ifeval": 0.10,
            "ailuminate": 0.05,
        },
    },
    {
        "id": "document_operations",
        "label": "Document Operations",
        "icon": "🗂️",
        "description": "Document review, form extraction, policy QA, and visual enterprise content workflows",
        "segment": "enterprise",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": [],
        "benchmark_notes": {
            "mmmu": "Low-confidence multimodal context for forms, charts, and screenshots.",
            "ifeval": "Low-confidence instruction-following context for extraction and policy QA.",
            "aa_intelligence": "General capability context for mixed document tasks.",
            "ailuminate": "Low-confidence safety context when document content is sensitive.",
            "aa_speed": "Throughput for bulk processing.",
        },
        "weights": {
            "aa_intelligence": 0.50,
            "aa_speed": 0.20,
            "mmmu": 0.15,
            "ailuminate": 0.10,
            "ifeval": 0.05,
        },
    },
    {
        "id": "knowledge_work_rag_sorting",
        "label": "Knowledge Work / RAG Retrieval Sorting",
        "icon": "🔎",
        "description": "Retrieval sorting, context ranking, and grounded answer shaping around retrieved context",
        "segment": "enterprise",
        "status": "preview",
        "min_coverage": 0.55,
        "required_benchmarks": [],
        "benchmark_notes": {
            "rag_groundedness": "Low-confidence groundedness context only; it is not a retrieval relevance eval.",
            "rag_task_faithfulness": "Low-confidence hallucination context only; it does not measure retrieval quality directly.",
            "ifeval": "Low-confidence instruction-following context only.",
            "aa_intelligence": "Primary capability anchor for synthesis and analysis once retrieval is assembled.",
            "chatbot_arena": "Answer quality after retrieval is assembled.",
            "gpqa_diamond": "Reasoning quality once retrieved material is available.",
        },
        "weights": {
            "aa_intelligence": 0.50,
            "chatbot_arena": 0.20,
            "gpqa_diamond": 0.20,
            "rag_task_faithfulness": 0.05,
            "rag_groundedness": 0.03,
            "ifeval": 0.02,
        },
    },
    {
        "id": "retrieval_embeddings",
        "label": "Retrieval Embeddings",
        "icon": "🔎",
        "description": "Embedding-model selection for semantic retrieval, vector search, and first-stage RAG recall",
        "segment": "enterprise",
        "status": "preview",
        "model_roles": ["embedding"],
        "min_coverage": 0.5,
        "required_benchmarks": ["mteb_retrieval"],
        "benchmark_notes": {
            "mteb_retrieval": "Primary retrieval quality signal from MTEB task-level main scores.",
            "mteb_retrieval_reranking": "Optional blended signal when the same model also has reranking coverage.",
            "rteb_finance": "Optional finance-domain retrieval quality signal from RTEB(fin, beta).",
        },
        "weights": {
            "mteb_retrieval": 0.65,
            "rteb_finance": 0.25,
            "mteb_retrieval_reranking": 0.10,
        },
    },
    {
        "id": "retrieval_reranking",
        "label": "Retrieval Reranking",
        "icon": "↕",
        "description": "Reranker-capable model selection for second-stage RAG ordering and search-result refinement",
        "segment": "enterprise",
        "status": "preview",
        "model_roles": ["reranker"],
        "min_coverage": 0.5,
        "required_benchmarks": ["mteb_reranking"],
        "benchmark_notes": {
            "mteb_reranking": "Primary reranking quality signal from MTEB task-level main scores.",
            "mteb_retrieval_reranking": "Optional blended context across retrieval and reranking tasks.",
        },
        "weights": {
            "mteb_reranking": 0.80,
            "mteb_retrieval_reranking": 0.20,
        },
    },
    {
        "id": "governed_enterprise_rollout",
        "label": "Governed Enterprise Rollout",
        "icon": "📜",
        "description": "Regulated deployment, procurement, and risk-sensitive rollout decisions",
        "segment": "enterprise",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": [],
        "benchmark_notes": {
            "ailuminate": "Low-confidence public safety context only.",
            "ifeval": "Low-confidence instruction-following context for policy and rollout controls.",
            "aa_intelligence": "General capability context for procurement tradeoffs.",
            "aa_cost": "Operational spend and vendor economics.",
        },
        "weights": {
            "aa_intelligence": 0.60,
            "aa_cost": 0.25,
            "ailuminate": 0.10,
            "ifeval": 0.05,
        },
    },
    {
        "id": "developer_platform_agent",
        "label": "Developer Platform Agent",
        "icon": "🧩",
        "description": "Internal engineering automation, repo tasks, and CI/CD support",
        "segment": "enterprise",
        "status": "preview",
        "min_coverage": 0.55,
        "required_benchmarks": ["terminal_bench", "swebench_verified"],
        "benchmark_notes": {
            "terminal_bench": "Agent-derived workflow evidence for terminal and tool execution.",
            "swebench_verified": "Repo-level bug fixing and code-change execution.",
            "ifeval": "Low-confidence instruction-following context for build and change-management steps.",
            "aa_intelligence": "General reasoning context for engineering workflows.",
        },
        "weights": {
            "terminal_bench": 0.40,
            "swebench_verified": 0.35,
            "aa_intelligence": 0.20,
            "ifeval": 0.05,
        },
    },
    {
        "id": "workflow_automation",
        "label": "Workflow Automation",
        "icon": "🔁",
        "description": "Back-office automation, queue handling, and multi-step ops workflows",
        "segment": "enterprise",
        "status": "preview",
        "min_coverage": 0.55,
        "required_benchmarks": ["terminal_bench"],
        "benchmark_notes": {
            "terminal_bench": "Agent-derived workflow evidence for multi-step execution.",
            "ifeval": "Low-confidence instruction-following context for operational procedures.",
            "ailuminate": "Low-confidence safety context for actions that touch business systems.",
            "aa_cost": "Operating cost for high-volume workflow execution.",
            "aa_speed": "Latency and throughput for queue-driven work.",
        },
        "weights": {
            "terminal_bench": 0.60,
            "aa_cost": 0.15,
            "aa_speed": 0.10,
            "ifeval": 0.10,
            "ailuminate": 0.05,
        },
    },
    {
        "id": "small_model_routing",
        "label": "Small-Model Routing",
        "icon": "🌱",
        "description": "When to use small, fast, and cheap models for triage and first-pass work",
        "segment": "enterprise",
        "status": "ready",
        "min_coverage": 0.5,
        "required_benchmarks": ["aa_cost", "aa_speed"],
        "benchmark_notes": {
            "aa_cost": "Primary economic signal for cheap first-pass routing.",
            "aa_speed": "Latency and throughput for triage and extraction.",
            "ifeval": "Low-confidence reliability context for formatting and routing rules.",
            "chatbot_arena": "Output quality when a small model handles first-pass work.",
            "ailuminate": "Low-confidence safety context only.",
        },
        "weights": {
            "aa_cost": 0.45,
            "aa_speed": 0.35,
            "chatbot_arena": 0.15,
            "ifeval": 0.03,
            "ailuminate": 0.02,
        },
    },
]

AILUMINATE_GRADE_TO_SCORE: dict[str, float] = {
    "Poor": 0.0,
    "Fair": 25.0,
    "Good": 50.0,
    "VeryGood": 75.0,
    "Very Good": 75.0,
    "Excellent": 100.0,
}

SEED_SCORES: list[dict[str, Any]] = [
    {"model_id": "gemini-3-1-pro", "benchmark_id": "aa_intelligence", "value": 57.0, "raw_value": "57"},
    {"model_id": "gpt-5-4", "benchmark_id": "aa_intelligence", "value": 57.0, "raw_value": "57"},
    {"model_id": "gpt-5-3-codex", "benchmark_id": "aa_intelligence", "value": 54.0, "raw_value": "54"},
    {"model_id": "claude-opus-4-6", "benchmark_id": "aa_intelligence", "value": 53.0, "raw_value": "53"},
    {"model_id": "claude-sonnet-4-6", "benchmark_id": "aa_intelligence", "value": 52.0, "raw_value": "52"},
    {"model_id": "glm-5", "benchmark_id": "aa_intelligence", "value": 50.0, "raw_value": "50"},
    {"model_id": "gemini-3-1-pro", "benchmark_id": "gpqa_diamond", "value": 94.1, "raw_value": "94.1%"},
    {"model_id": "gpt-5-4", "benchmark_id": "gpqa_diamond", "value": 92.0, "raw_value": "92.0%"},
    {"model_id": "gpt-5-3-codex", "benchmark_id": "gpqa_diamond", "value": 91.5, "raw_value": "91.5%"},
    {
        "model_id": "claude-opus-4-6",
        "benchmark_id": "gpqa_diamond",
        "value": 91.3,
        "raw_value": "~91.3%",
        "source_type": "secondary",
        "verified": 0,
        "notes": "From aggregator - verify against primary",
    },
    {"model_id": "o4-mini-high", "benchmark_id": "mmmu", "value": 79.2, "raw_value": "79.2%"},
    {"model_id": "gpt-5-4", "benchmark_id": "mmmu", "value": 79.1, "raw_value": "79.1%"},
    {
        "model_id": "claude-opus-4-6",
        "benchmark_id": "swebench_verified",
        "value": 80.8,
        "raw_value": "80.8%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "gemini-3-1-pro",
        "benchmark_id": "swebench_verified",
        "value": 80.6,
        "raw_value": "80.6%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "claude-sonnet-4-6",
        "benchmark_id": "swebench_verified",
        "value": 79.6,
        "raw_value": "79.6%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "glm-5",
        "benchmark_id": "swebench_verified",
        "value": 77.8,
        "raw_value": "77.8%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "gemini-3-1-pro",
        "benchmark_id": "terminal_bench",
        "value": 78.4,
        "raw_value": "78.4%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "gpt-5-3-codex",
        "benchmark_id": "terminal_bench",
        "value": 77.3,
        "raw_value": "77.3%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "claude-opus-4-6",
        "benchmark_id": "terminal_bench",
        "value": 74.7,
        "raw_value": "74.7%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "glm-5",
        "benchmark_id": "ifeval",
        "value": 88.0,
        "raw_value": "88.0%",
        "source_type": "secondary",
        "verified": 0,
    },
    {
        "model_id": "glm-5",
        "benchmark_id": "chatbot_arena",
        "value": 1451.0,
        "raw_value": "1451",
        "source_type": "secondary",
        "verified": 0,
    },
    {"model_id": "mercury-2", "benchmark_id": "aa_speed", "value": 789.2, "raw_value": "789.2 t/s"},
    {"model_id": "gemini-2-5-flash", "benchmark_id": "aa_speed", "value": 386.1, "raw_value": "386.1 t/s"},
    {"model_id": "gemma-3n-e4b", "benchmark_id": "aa_cost", "value": 0.03, "raw_value": "$0.03/1M"},
]


def _has_rows(conn: Connection, table) -> bool:
    count = conn.execute(select(func.count()).select_from(table)).scalar_one()
    return bool(count)


def _upsert_rows(
    conn: Connection,
    table,
    rows: Iterable[Mapping[str, Any]],
    *,
    preserve_columns: Iterable[str] = (),
) -> None:
    rows_list = list(rows)
    if not rows_list:
        return

    stmt = sqlite_insert(table).values(rows_list)
    preserved = set(preserve_columns)
    update_columns = {
        column.name: getattr(stmt.excluded, column.name)
        for column in table.columns
        if column.name != "id" and column.name not in preserved
    }
    stmt = stmt.on_conflict_do_update(index_elements=["id"], set_=update_columns)
    conn.execute(stmt)


def _insert_rows_if_missing(conn: Connection, table, rows: Iterable[Mapping[str, Any]]) -> None:
    rows_list = list(rows)
    if not rows_list:
        return

    stmt = sqlite_insert(table).values(rows_list)
    stmt = stmt.on_conflict_do_nothing(index_elements=["id"])
    conn.execute(stmt)


def seed_reference_data(target: Connection | Engine, *, include_seed_scores: bool = False) -> None:
    if isinstance(target, Engine):
        with target.begin() as conn:
            seed_reference_data(conn, include_seed_scores=include_seed_scores)
        return

    conn = target
    _upsert_rows(conn, benchmarks, BENCHMARKS)
    _insert_rows_if_missing(conn, providers, PROVIDERS)
    apply_provider_origin_baseline(conn)
    _upsert_rows(
        conn,
        models,
        [
            {
                **row,
                "provider_id": row.get("provider_id") or provider_id_from_name(str(row.get("provider") or "")),
                "model_roles_json": row.get("model_roles_json") or json.dumps(["generator"], ensure_ascii=True),
            }
            for row in MODELS
        ],
        preserve_columns=(
            "general_approved_for_use",
            "general_approval_notes",
            "general_approval_updated_at",
            "approved_for_use",
            "approval_notes",
            "approval_updated_at",
        ),
    )

    if include_seed_scores and not _has_rows(conn, scores):
        collected_at = "2026-03-31T00:00:00Z"
        rows = []
        for row in SEED_SCORES:
            payload = dict(row)
            payload.setdefault("source_type", "primary")
            payload.setdefault("verified", 1)
            payload.setdefault("source_url", None)
            payload.setdefault("notes", None)
            payload.setdefault("collected_at", collected_at)
            rows.append(payload)
        conn.execute(scores.insert(), rows)


__all__ = [
    "AILUMINATE_GRADE_TO_SCORE",
    "BENCHMARKS",
    "INTERNAL_VIEW_BENCHMARK_ID",
    "MODELS",
    "PROVIDERS",
    "PROVIDER_ORIGIN_BASELINE_PATH",
    "SEED_SCORES",
    "USE_CASES",
    "apply_provider_origin_baseline",
    "canonical_provider_name",
    "derive_provider_origin_fields",
    "export_provider_origin_baseline",
    "load_provider_origin_baseline",
    "normalize_origin_countries",
    "provider_id_from_name",
    "seed_reference_data",
]
