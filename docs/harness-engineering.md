...
| Task packet | `make agent-task-packet`, `schemas/task-packet.schema.json` | story context, non-goals, allowed files, protected files, goal alignment, test strategy, validation, exact docs and release metadata surfaces, docs impact, risk, approval state, and optional approved learning-decision lineage | task packet JSON or Markdown handoff; lineage does not change target task files or receipt mechanics |
...
| Session receipt | `make agent-receipt-verify`, receipt schema | commands, selected test boundary, e2e-required evidence or skip reason, skipped checks, docs impact, findings, disposition | strict receipt validation |
...
| Supervised learning records | `kit learn status`, event/proposal/decision/context routes, `kit learn thread-summary import/list`, `kit learn upstream export/list/reconcile`, `kit learn evaluate`, `.agent-workflows/learning-policy.json`, `schemas/learning-*.schema.json` | target-owned policy gates bounded local evidence, one explicit redacted aggregate import, and portable source-review candidates | import never scans history or raw interaction content and writes one approved sidecar event only; upstream export needs current approved lineage, redaction confirmation, and public-ok/internal privacy; lists/evaluate/reconcile are read-only, revalidate lineage, and never propagate changes |
