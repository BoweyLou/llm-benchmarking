...
- [x] LBM-073: P1 Add comparable benchmark displays across model types
  - Source: Human request 2026-07-15 after a raw MTEB score in the review surface provided no reference point for judging it against other imported models.
  - Problem: benchmark values have source-specific units, directions, ranges, configurations, and evidence depth, but the API and review UI exposed them without a shared presentation or comparison contract.
  - Scope: add a server-owned policy registry for all 92 code-owned benchmarks with historical retirement, role-, configuration-, and evaluation-signature-aware strict and broad cohorts, deterministic canonical evidence arbitration, formatted values, raw-value rank/tie semantics, coverage and evidence context across APIs/UI/rankings/exports, active-use-case missing evidence, exact configured-duplicate suppression, corrected SWE-bench normalization, and uncapped fail-closed MTEB enumeration with coherent revisions and split/subset signatures.
  - Acceptance: every active benchmark has exactly one typed policy and retired database-only definitions are inactive without history loss; raw values remain stored unchanged while display values carry units and direction; rank/ties use stored normalized numerics; strict comparisons isolate configurations/signatures while broad comparisons warn about mixed evidence; the review surface shows four relevance-selected Key benchmarks, expandable complete evidence, and active-use-case missing evidence; grouped aliases retain complementary evidence without duplicating the latest configured observation; partial MTEB refreshes preserve prior scores; invalid bounded results are excluded; API/export consumers receive comparison context; accessible desktop and mobile states remain usable.
  - Validation: 287-test inference suite; policy/API/ranking/export/source regressions; constant-query comparison-cache regression; docs/version gates; deployed schema-4 APIs and every JSON/JSONL/CSV/banking export; responsive desktop/mobile browser QA across generator, embedding/reranker, speech-to-text, text-to-speech, lower-is-better, populated, and empty-evidence states; Proxmox update log 38 with 12,566 MTEB files probed, zero unresolved failures, 611 complete RTEB rows, 277 scores added, and 364 updated.
  - Completed: 2026-07-15. Benchmark values now carry server-owned formatting, cohort position, evidence depth, coverage, and provenance across APIs, exports, rankings, and the review workbench; SWE-bench and MTEB source defects are corrected and live.
