...
1. Promote metadata already fetched by adapters. Chatbot Arena and IFEval already
...
8. Expand official SWE-bench coverage beyond Verified. The public SWE-bench
   ...
   metadata.
9. Expose source freshness and degradation in exports. Done for active benchmark
   source runs through each model's `source_freshness` list, including stale and
   missing-source-failed markers.
